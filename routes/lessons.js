...
// Get all public lessons with pagination, filtering, and sorting
...
router.get("/similar-lessons/:id", async (req, res) => {
  try {
    ...
    console.error("Error fetching similar lessons:", error.message);
    res.status(500).json({ message: "Internal server error" });
  }
});
...
// Get lessons by creator (for profile pages)
router.get("/creator/:creatorId", async (req, res) => {
  try {
    ...
    const lessons = await lessonsCollection
      .find({ creatorId, visibility: "public" })
      .skip(skip)
      .limit(parseInt(limit))
      .sort({ createdAt: -1 })
      .project({
    ...
    console.error("Error fetching creator lessons:", error.message);
    res.status(500).json({ message: "Internal server error" });
  }
});
